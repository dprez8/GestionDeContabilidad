...
class RequestHelper {
	...
	*/
	...
	/**
	* Crear un request de tipo PATCH a `path` con `data` y ejecuta los distintos callbacks por contexto
	* @param {String} path The name to say hi to
	* @param {Object} data The name to say hi to
	* @param {Object} callback The name to say hi to
	*/
	static patch(path, data, callback, config) {
		var token = sessionStorage.getItem('token');
		if(token) {
			if(config) {
				config.headers = {
